...
with lib; let
  inherit (origin.config) meta;
  cfg = config.impure-config-management;

  copy = n: v: {source = meta.flakeRoot + "/${v}";};
    ...
    impure-config-management = {
      symlink.enable = mkOption {
        ...
        description = ''
          ...
          `impure-config-management.symlink.base` to the location where you cloned the flake.
        '';
      };
