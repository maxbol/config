...
in {
  ...
  config = mkMerge [
    ...
    (mkIf (cfg.enable && cfg.bat.enable) {
      ...
      xdg.configFile."bat/config".source = mkOverride 1 (config.lib.file.mkOutOfStoreSymlink "${config.theme-config.themeDirectory}/active/bat/config");
    })
  ];
}
