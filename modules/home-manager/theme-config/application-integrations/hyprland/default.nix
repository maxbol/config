...
in {
  options = {
    theme-config.hyprland.enable = mkOption {
      type = types.bool;
  ...
  config = {
    assertions = [
      {
        assertion = !(cfg.enable && config.theme-config.hyprland.enable) || config.wayland.windowManager.hyprland.enable;
        message = "Chroma's Hyprland integration only works when the Hyprland module is enabled";
      }
      {
        assertion = !(cfg.enable && config.theme-config.hyprland.enable) || config.theme-config.desktop.enable;
        message = "Chroma's desktop module is required for the Hyprland module.";
      }
    ];

    theme-config.programs.hyprland = {
      activationCommand = {
        name,
        opts,
      }: let
            ...
            package = let
              translate = theme:
                pkgs.stdenv.mkDerivation {
                  ...
                  nativeBuildInputs = [vendor.hyprcursor.default pkgs.xcur2png];
    ...
    wayland.windowManager.hyprland = mkIf (cfg.enable && config.theme-config.hyprland.enable) {
      settings = {
    ...
    home.packages =
      mkIf (cfg.enable && config.theme-config.hyprland.enable)
