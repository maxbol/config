...
in {
  options = {
    theme-config.kitty.enable = mkOption {
      type = types.bool;
  ...
  config = {
    ...
    theme-config.programs.kitty = let
      pkill =
    ...
    programs.kitty.extraConfig = mkIf (cfg.enable && cfg.kitty.enable) ''
      include ${config.theme-config.themeDirectory}/active/kitty/theme.conf
      include ${config.theme-config.themeDirectory}/active/kitty/fonts.conf
    '';
    # programs.kitty.settings.include = mkIf (cfg.enable && cfg.kitty.enable) "${config.theme-config.themeDirectory}/active/kitty/theme.conf";
  };
