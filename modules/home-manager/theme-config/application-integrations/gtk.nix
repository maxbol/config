...
in {
  options = {
    theme-config.gtk = {
      ...
      gtk2.extraConfig = mkOption {
        ...
        default =
          options.theme-config.gtk.gtk3.extraConfig.default
          // {
      ...
      gtk3.extraConfig = mkOption {
        type = options.theme-config.gtk.gtk2.extraConfig.type;
        # TODO: move the default settings elsewhere
        default = {
      ...
      gtk4 = {
        ...
        extraConfig =
          options.theme-config.gtk.gtk3.extraConfig
          // {
  ...
  config = {
    theme-config.programs.gtk = {
      ...
      reloadCommand = "${lib.getExe self.gtkrc-reload}";
      ...
      home.replaceRuntimeDependencies = mkIf (cfg.gtk.gtk4.libadwaitaSupport == "patch-binary") [
        {
          ...
          replacement = self.libadwaita-without-adwaita;
        }
      ];
    })
  ];
