...
roconf="${XDG_CONFIG_HOME:-$HOME/.config}/rofi/quickapps.rasi"
...
DATA_DIRS="${XDG_DATA_DIRS:-/usr/share:/usr/local/share}:$HOME/.local/share"
APP_DIRS="$(for dir in ${DATA_DIRS//:/ }; do test -d "$dir/applications" && echo -n "$dir/applications "; done)"
DESKTOP_FILES="$(find -L $APP_DIRS -type f -iname '*.desktop')"
RofiSel=$( for qapp in "$@"
do
    Lkp=`grep "$qapp" $DESKTOP_FILES | grep 'Exec=' | awk -F ':' '{print $1}' | head -1`
